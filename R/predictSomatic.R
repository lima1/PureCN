...
### This function takes as input the output of a runAbsoluteCN run and annotates variants
### more correctly as germline vs. somatic by inferring maternal and paternal chromosome numbers.
res, 
...
### Exclude maternal/paternal chromosome number in segment if posterior probability is lower than cutoff.
   ...
   # llik.segment <- split(as.data.frame(llik), res$results[[id]]$SNV.posterior$beta.model$segment.ids)
   # germline.cols <- grep("GERMLINE.M",colnames(llik))
   # llik.segment.sums <- lapply(llik.segment, function(x) colSums(x[, germline.cols])/max(colSums(x[, germline.cols])))
    llik.segment.sums <- .calcMpriorGermline(res$results[[id]]$SNV.posterior$beta.model)
    ...
    for (i in names(llik.segment.delete)) {
         row.ids <- which(res$results[[id]]$SNV.posterior$beta.model$segment.ids == i)
    ...
    # set everything that does not match well and has low allelic fraction as somatic 
    ...
    pp.new$Cellfraction <-  (pp.new$AR/pp.new$ML.M)*(purity*pp.new$ML.C+2*(1-purity))/purity
...
# no matched normal is available, this will help predicting somatic vs. germline
# status
...
.calcMpriorGermline <- function(model) {
    ...
    germline.cols <- grep("GERMLINE.M",colnames(llik))
    lapply(llik.segment, function(x) colSums(x[, germline.cols])/max(colSums(x[, germline.cols])))
}
