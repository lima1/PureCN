test_that("test_runAbsoluteCN", {
    ...
    expect_error(callLOH(ret))
    expect_true(grepl("runAbsoluteCN was run without a VCF file", 
        geterrmessage()))
    ...
    expect_error(runAbsoluteCN(tumor.coverage.file = tumor.coverage.file, 
        genome = "hg19"))
    expect_true(grepl("Need a normal coverage file if log.ratio and seg.file", 
        geterrmessage()))
    expect_error(runAbsoluteCN(tumor.coverage.file = tumor.coverage.file, 
        min.ploidy = 0, genome = "hg19"))
    expect_true(grepl("min.ploidy or max.ploidy not within expected range", 
        geterrmessage()))
    expect_error(runAbsoluteCN(tumor.coverage.file = tumor.coverage.file, 
    ...
    expect_error(runAbsoluteCN(tumor.coverage.file, tumor.coverage.file, 
        genome = "hg19"))
    expect_true(grepl("Tumor and normal are identical", geterrmessage()))
    expect_error(runAbsoluteCN(tumor.coverage.file = tumor.coverage.file, 
        log.ratio = head(purecn.example.output$input$log.ratio$log.ratio), 
        genome = "hg19"))
    expect_true(grepl("Length of log.ratio different from tumor coverage", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        prior.purity = 1, genome = "hg19"))
    expect_true(grepl("prior.purity must have the same", geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        min.gof = 70, genome = "hg19"))
    expect_true(grepl("min.gof", geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        prior.purity = c(0.6, 1.1), test.purity = c(0.2, 0.6), 
        genome = "hg19"))
    expect_true(grepl("prior.purity not within expected range", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        prior.purity = c(0.6, 0.9), test.purity = c(0.2, 0.6), 
        genome = "hg19"))
    expect_true(grepl("prior.purity must add to 1. Sum is 1.5", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        genome = "hg19", max.homozygous.loss = 1.1))
    expect_true(grepl("max.homozygous.loss not within expected range", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        genome = "hg19", prior.K = -1.1))
    expect_true(grepl("prior.K not within expected range", geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        genome = "hg19", prior.contamination = c(0.1, -1.1)))
    expect_true(grepl("prior.contamination not within expected range", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        genome = "hg19", iterations = 1))
    expect_true(grepl("Iterations not in the expected range from", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
    ...
    expect_true(grepl("Iterations not in the expected range from", 
        geterrmessage()))
    expect_error(runAbsoluteCN(normal.coverage.file, tumor.coverage.file, 
        genome = "hg19", model.homozygous = NULL))
    expect_true(grepl("model.homozygous", geterrmessage()))
    ...
    expect_true(grepl("Interval files in normal and tumor different", 
        geterrmessage()))
    ret <- runAbsoluteCN(normal.coverage.file = normal.coverage.file, 
    ...
    write.table(gc2, file = "tmp.gc", row.names = FALSE, sep = "\t", 
        quote = FALSE)
    ret <- runAbsoluteCN(normal.coverage.file = normal.coverage.file, 
        gc.gene.file = "tmp.gc", model.homozygous = TRUE, tumor.coverage.file = tumor.coverage.file, 
    ...
    expect_error(callAlterations(ret))
    ...
    expect_true(grepl("Different chromosome names in coverage and VCF", 
        geterrmessage()))
    ...
    expect_true(grepl("tumor.coverage.file and gc.gene.file do not", 
        geterrmessage()))
    gc3 <- read.delim(gc.gene.file, as.is = TRUE)
    gc3[, 1] <- as.character(tumorCov)
    write.table(gc3, file = "tmp3.gc", row.names = FALSE, sep = "\t", 
        quote = FALSE)
    gc4 <- gc3
    gc4$Gene[2:4] <- paste("SCNN1D,XXXL")
    write.table(gc4, file = "tmp4.gc", row.names = FALSE, sep = "\t", 
        quote = FALSE)
    set.seed(123)
    ret <- runAbsoluteCN(normal.coverage.file = normCov, tumor.coverage.file = tumorCov, 
        gc.gene.file = "tmp3.gc", plot.cnv = FALSE, min.ploidy = 1.4, 
    ...
    ret2 <- runAbsoluteCN(normal.coverage.file = normCov, tumor.coverage.file = tumorCov, 
        gc.gene.file = "tmp4.gc", plot.cnv = FALSE, min.ploidy = 1.4, 
        max.ploidy = 2.4, vcf.file = vcf, genome = "hg19", test.purity = seq(0.4, 
            0.7, by = 0.05), max.candidate.solutions = 1)
    ...
    write.table(tmp, file = "seg_wrong.tmp", quote = FALSE, row.names = FALSE, 
        sep = "\t")
    expect_error(runAbsoluteCN(seg.file = "seg_wrong.tmp", gc.gene.file = gc.gene.file, 
        vcf.file = vcf.file, max.candidate.solutions = 1, genome = "hg19", 
    ...
    write.table(tmp, file = "seg.tmp", quote = FALSE, row.names = FALSE, 
        sep = "\t")
    expect_error(runAbsoluteCN(seg.file = "seg.tmp", gc.gene.file = gc.gene.file, 
        vcf.file = vcf.file, max.candidate.solutions = 1, genome = "hg19", 
        min.ploidy = 1.4, max.ploidy = 2.4, test.purity = seq(0.4, 
            0.7, by = 0.05), verbose = FALSE))
    expect_true(grepl("seg.file contains multiple samples and sampleid missing", 
        geterrmessage()))
    expect_error(runAbsoluteCN(seg.file = "seg.tmp", gc.gene.file = gc.gene.file, 
        sampleid = "Sample3", vcf.file = vcf.file, max.candidate.solutions = 1, 
        genome = "hg19", test.purity = seq(0.3, 0.7, by = 0.05), 
        verbose = FALSE))
    expect_true(grepl("contains multiple samples and sampleid does not match", 
        geterrmessage()))
    ret <- runAbsoluteCN(seg.file = "seg.tmp", gc.gene.file = gc.gene.file, 
    ...
    expect_error(runAbsoluteCN(normal.coverage.file = normal.coverage.file, 
        tumor.coverage.file = tumor.coverage.file, genome = "hg19", 
        normalDB = tmp))
    expect_true(grepl("normalDB appears to be empty", geterrmessage()))
    ret <- runAbsoluteCN(normal.coverage.file = normal.coverage.file, 
        tumor.coverage.file = tumor.coverage.file, genome = "hg19", 
    ...
    write.table(gc.data, file = tmpFile, row.names = FALSE, quote = FALSE, 
        sep = "\t")
    ret <- runAbsoluteCN(normal.coverage.file = normal.coverage.file, 
        tumor.coverage.file = tumor.coverage.file, candidates = purecn.example.output$candidates, 
    ...
    x <- runAbsoluteCN(normal.coverage.file, tumor, genome = "hg19", 
        min.ploidy = 1.4, max.ploidy = 2.4, test.purity = seq(0.4, 
            0.7, by = 0.05), max.candidate.solutions = 1, gc.gene.file = gc.gene.file)
})
