...
\section*{Prepare Environment and Files}
...
\begin{verbatim}
$ Rscript $PURECN/IntervalFile.R --bed ex_intervals.bed --fasta ex_reference.fa \ 
 --outfile ex_gcgene.txt
\end{verbatim}

See the main vignette how to add gene symbols, which are necessary to obtain
gene-level copy number and LOH calls. For a test run, you will not need this.

\section*{Run PureCN}

\subsection*{Third-party segmentation}
...
The \software{MuTect} stats file can be provided for better artifact filtering.
...
The \Rcode{--funsegmentation} controls if the data should to be re-segmented
using germline BAFs (default). Set this value to none if the provided
segmentation should be used as is. 

\subsection*{PureCN segmentation}

The following describe \Biocpkg{PureCN} runs with internal copy number
...
\begin{verbatim}
cat normal*loess.txt > example_normal.list
...
$ Rscript $PURECN/PureCN.R --outdir . --tumor example_tumor_loess.txt \ 
    --normal example_normal_loess.txt --vcf $VCF_FILE -i Sample1 \
    --genome hg19 --gcgene  ex_gcgene.txt \
    --outvcf 

# Without a matched normal    
$ Rscript $PURECN/PureCN.R --outdir . --tumor example_tumor_loess.txt \ 
    --normaldb ../normalDB_hg19.rds --pool 5 --vcf example_vcf.vcf -i Sample1 \
