...
\begin{verbatim}
$ Rscript $PURECN/IntervalFile.R --infile ex_intervals.bed --fasta ex_reference.fa \ 
 --outfile ex_gcgene.txt
\end{verbatim}

Internally, this function uses \Biocpkg{rtracklayer} to parse the
\Rcode{infile}. Make sure that the file format matches the file name suffix. 
