...
can skip this argument or simply download the file from the UCSC website (see
the FAQ section of the main vignette for instruction how to generate such
a file).
\end{itemize}
...
To build a normal database, copy the paths to all GC-normalized normal coverage
files in a single text file, line-by-line:
...
\begin{itemize}
    \item The resulting \Rcode{normalDB\_hg19.rds} file contains absolute paths
    to the coverage files. It is thus necessary to re-run this command when the
    coverage files are moved.
...
--offtargetseqlevels & off.target.seqlevels & \Rfunction{calculateGCContentByInterval} \\
