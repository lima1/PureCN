...
the modified intervals as BED file for example (again every rtracklayer format
is supported). This is useful when the coverages are calculated with
third-party tools like GATK.
...
\begin{itemize}
    ...
    \item The \Rcode{--pool} flag specifies how many normal samples should be
        ...
        significant, e.g. for samples with different read sizes or insert size
        distributions.
