...
On-target regions with low mappability will be excluded.
...
Provided are again minimal examples for test runs. See the main vignette how to
get optimal results in production pipelines.
