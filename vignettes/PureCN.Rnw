...
PureCN was designed to integrate well with industry standard pipelines, but it
is straightforward to generate input data from other pipelines.
...
from the \Biocpkg{TitanCNA} package, can be used to correct for GC bias. We
...
females). Non-diploid chromosomes provide only limited information for purity
and ploidy selection, but are of interest for copy number calling. Future
versions might support full copy number calling on the XY chromosomes.
