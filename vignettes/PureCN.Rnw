...
designed for hybrid caputure sequencing data, especially with medium-sized
...
It is recommended to GC-normalize coverage. GC-content is provided via the
gc.gene.file argument. The expected format:
...
can be used to generate the \Rcode{gc\_bias} column. The \Rcode{Gene} column is
optional and only required for providing gene-level copy number and LOH calls. 
...
\subsection{Short nucleotide variants}
...
The number of considered local optima is typically high. Optima with low
likelihood score can be automatically removed with the
\Rfunction{bootstrapSolutions} function
(Figure~\ref{figure/figureexample1b-1}).  This function bootstraps variants in
the provided VCF and re-ranks local optima. Solutions that never rank high are
then excluded. Large-scale copy number artifacts can decrease the likelihood
scores of correct solutions, potentially resulting in a removal of
purity/ploidy estimates closest to the true values.

<<figureexample1b, fig.show='hide'>>=
ret <- bootstrapSolutions(ret)
plotAbs(ret, type="overview")
@
